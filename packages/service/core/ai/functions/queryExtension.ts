...
`
  ...
  //console.log('queryExtension_query:',query);
  const messages = [
    {
      role: 'user',
  ...
  //console.log('queryExtension_messages:',messages);
  ...
  //console.log('queryExtension_lay:',result.choices?.[0]?.message);
  ...
  try {
    ...
    return {
      rawQuery: query,
      extensionQueries: Array.isArray(queries) ? queries : [query],
      model,
