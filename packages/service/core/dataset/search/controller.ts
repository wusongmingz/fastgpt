...
export async function searchDatasetData(props: SearchDatasetDataProps) {
  ...
  console.log('知识库检索核心函数内部_datasetIds：',datasetIds);
  console.log('知识库检索核心函数内部_queries：',queries);
  ...
  */
  ...
  const filterMaxTokensResult = await (async () => {
    ...
    let totalTokens = 0;
    let graphdatasetIdStats = new Set<string>(); // 图知识库统计器
    for await (const item of tokensScoreFilter) {
      const datasetInfo :any= await MongoDataset.findOne({ _id: item.datasetId });
      console.log('初步索引结果：',item);
      if (graphdatasetIdStats && graphdatasetIdStats.has(item.datasetId) ){//图知识库只检索一次
        break
      }
      if (datasetInfo && datasetInfo.type === 'graph') {//图知识库逻辑
        console.log('知识库类型_lay：',datasetInfo.type);
        graphdatasetIdStats.add(item.datasetId); 
        totalTokens += item.tokens;
        if (totalTokens > maxTokens + 500) {
          break;
        }
        console.log('图知识库检索核心函数内部结果_item：',item);
        const datasetCollectionInfo = await MongoDatasetCollection.findOne({
          datasetId: item.datasetId
        });
        if (!datasetCollectionInfo || !datasetCollectionInfo.fileId) {
          return {
            message: '图知识库的数据库查找失败'
          };
        }
        const fileid = datasetCollectionInfo.fileId;
        const base = process.env.GRAPH_URL || 'http://127.0.0.1:8001';
        const url = `${base}/chat/completions`;//图知识库检索
        const response_graph: any = await POST(url, {
          fileId: fileid, 
          content: String(queries),
          query_options: {
            query_type: searchMode
          }
        });
        
        console.log('response_graph_qa：', response_graph);
        if(!response_graph.data.llm_data){
          item.q = ''
        }else{item.q =response_graph.data.llm_data}
        
        item.graph_response = response_graph;
        results.push(item);
      ...
      console.log('非图知识库检索核心函数内部结果_item：',item);
      results.push(item);
      if (totalTokens > maxTokens) {
        break;
      }
  ...
  console.log('知识库检索核心函数内部结果_filterMaxTokensResult：',filterMaxTokensResult);
  return {
