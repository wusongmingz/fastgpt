...
const EditResourceModal = ({
  ...
  const { runAsync: onSave, loading } = useRequest2(
    (data: EditResourceInfoFormType) => onEdit(data),
    {
      onSuccess: (res) => {
        toast({
          title: t('common:common.rename success'),
          status: 'success'
        });
        onClose();
      }
  ...
  return (
    <MyModal
      ...
      title={
        <HStack ml={-2}>
          <Image src="/imgs/images/picture/rename.png" w={'25px'} alt=""></Image>
          <Text>{t('common:common.Rename')}</Text>
        </HStack>
      }
    >
      <ModalBody>
      ...
      <ModalFooter borderTop={'1px solid #e2e3ea'} padding={'5px'}>
        <Button
          isLoading={loading}
          variant={'whiteCommon'}
          w={'70px'}
