...
const ChatBox = (
  ...
  const { setLoading, feConfigs, llmModelList } = useSystemStore();
  ...
  const getModelAvatar = (i: any, llmModelList: any) => {
    // const { setLoading, feConfigs, llmModelList } = useSystemStore();
    //todo 暂时有问题、后续做调整name和model统一
    if (i.aiChatModel) {
      return llmModelList.find((item: any) => item.model === i.aiChatModel)?.avatar || appAvatar;
    }
    if (i.status === 'finish' && !!i.responseData) {
      return (
        llmModelList.find(
          (item: any) =>
            item.name ===
            i.responseData?.find((item) => {
              return item.moduleType == FlowNodeTypeEnum.chatNode;
            }).model
        )?.avatar || appAvatar
      );
    }
  ...
  const getModelName = (i: any, llmModelList: any) => {
    if (i.aiChatModel) {
      return llmModelList.find((item: any) => item.model === i.aiChatModel)?.name;
    }
    if (i.status === 'finish' && !!i.responseData) {
      return (
        i.responseData?.find((item: ChatHistoryItemResType) => {
          return item.moduleType === FlowNodeTypeEnum.chatNode;
        })?.model || aiName
      );
    }
                  ...
                  <Box key={item.dataId} py={6}>
                    ...
                    {item.obj === ChatRoleEnum.AI && (
                      <>
                        <ChatItem
                          type={item.obj}
                          // avatar={getModelAvatar(item, llmModelList)}
                          avatar={'/imgs/images/picture/bigmodel.png'}
                          chat={item}
                          aiName={getModelName(item, llmModelList)}
                          isLastChild={index === chatHistories.length - 1}
                          {...{
                            showVoiceIcon,
                            shareId,
