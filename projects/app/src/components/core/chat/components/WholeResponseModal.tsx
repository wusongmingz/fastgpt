...
import { Box, Flex, BoxProps, useDisclosure, HStack, Stack, Button } from '@chakra-ui/react';
...
export const WholeResponseContent = ({
  ...
  const RowRender = useCallback(
    ({
      ...
      return (
        <Flex mb={3}>
          <Box pr={2} fontSize={'sm'} pt={2} mb={mb} color={'myGray.800'} flex={'0 0 110px'}>
            {label}:
          </Box>
          <Box borderRadius={'lg'} minW={'300px'} fontSize={['xs', 'sm']} bg={'#eceff6'} {...props}>
            {children}
          </Box>
        </Flex>
      );
      ...
      <>
        ...
        {activeModule?.historyPreview && (
          <Flex direction={'column'}>
            <Box pr={4} fontSize={'sm'} mb={1} color={'myGray.800'} w={'120px'}>
              {t('common:core.chat.response.module historyPreview')}:
            </Box>
            <Box>
              {activeModule.historyPreview ? (
                <Box px={3} py={2} border={'base'} bg={'#f7f8fa'} borderRadius={'md'}>
    ...
    ({
      ...
      return (
        <Flex
          ...
          borderWidth={'1px'}
          borderColor={value === sideBarItem.id ? '#d7000f' : '#b7babf'}
          _hover={{ borderColor: '#d7000f' }}
          p={2}
          mb={2}
          width={'100%'}
          bg={'#fff'}
    ...
    <>
      {isPc && !useMobile ? (
        <Flex overflow={'hidden'} height={'100%'}>
          ...
          <Box flex={'6 0 0'} w={0} height={'100%'}>
            <WholeResponseContent activeModule={activeModule} hideTabs={hideTabs} />
          </Box>
        </Flex>
      ) : (
        <Box h={'100%'} w={'100%'} overflow={'auto'} className="111">
          {!isOpenMobileModal && (
            <WholeResponseSideTab
              response={sliderResponseList}
              value={currentNodeId}
