...
export function addStatisticalDataToHistoryItem(historyItem: ChatItemType) {
  ...
  const flatResData: ChatHistoryItemResType[] =
    historyItem.responseData
      ...
      .flat() || [];
  const datasetSearchNodeQuoteList = historyItem.responseData
    ?.find((item) => {
      return item.moduleType == FlowNodeTypeEnum.datasetSearchNode;
    })
    ?.quoteList?.filter((item) => !!item.graph_response);
  const tokensSum = historyItem.responseData?.reduce((sum, item) => {
    if (item.tokens) {
      return sum + item.tokens;
    } else if (item.toolCallTokens) {
      return sum + item.toolCallTokens;
    } else {
      return sum;
    }
  }, 0);
