...
import { AppFolderTypeList, AppTypeEnum, sortTypeEnum } from '@fastgpt/global/core/app/constants';
...
async function handler(req: ApiRequestProps<ListAppBody>): Promise<AppListItemType[]> {
  const { parentId, type, getRecentlyChat, searchKey, sortType } = req.body;
  ...
  const [myApps, perList] = await Promise.all([
    MongoApp.find(
      findAppsQuery,
      '_id parentId avatar type name intro tmbId selectColor updateTime pluginData inheritPermission'
    )
      .sort({
        updateTime: -1
      })
  ...
  const order = ['folder', 'simple', 'advanced', 'plugin', 'httpPlugin'];
  if (!!sortType) {
    if (sortType === sortTypeEnum.fileType) {
      filterApps.sort((a, b) => {
        return order.indexOf(a.type) - order.indexOf(b.type);
      });
    } else if (sortType == sortTypeEnum.updateTime) {
      filterApps.sort((a, b) => {
