...
async function handler(
  ...
  const { appId, shareId, outLinkUid, teamId, teamToken, offset, pageSize, source ,searchName} =
    req.body as getHistoriesBody;
  console.log('getHistoriesSearchreq.body_lay:',req.body);
  ...
  // 如果有searchName，进行后续筛选
  if (searchName) {
    data = data.filter(item => 
      item.title && item.title.toLowerCase().includes(searchName.toLowerCase())
    );
  }
  console.log('应用中心data：',data);
  return {
    list: data.map((item) => ({
      chatId: item.chatId,
