...
const Share = ({ appId }: { appId: string; type: PublishChannelEnum }) => {
  ...
  return (
    <MyBox isLoading={isFetching} mt={3} position={'relative'} h={'calc(100vh - 340px)'}>
      <Flex justifyContent={'space-between'} borderBottom={'1px solid #e0e0e0'} pb={1}>
        <HStack>
      ...
      <TableContainer mt={2}>
        <Table variant={'simple'} w={'100%'} overflowX={'auto'} fontSize={'sm'}>
          <Thead borderRadius={'none !important'}>
            <Tr bg={'#e2e3ea !important'}>
              <Th textAlign={'center'}>{t('common:common.Name')}</Th>
              {feConfigs?.isPlus && (
                <>
              ...
              <Th textAlign={'center'}>{t('common:core.app.share.Is response quote')}</Th>
              {feConfigs?.isPlus && (
                <>
                  <Th textAlign={'center'}>{t('common:core.app.share.Ip limit title')}</Th>
                  <Th textAlign={'center'}>{t('common:core.app.share.Role check')}</Th>
                </>
              )}
              ...
              <Tr key={item._id}>
                ...
                {feConfigs?.isPlus && (
                  <>
                    ...
                    <Th pt={'12px'} textAlign={'center'}>
                      {item?.limit?.hookUrl ? '是' : '否'}
                    </Th>
                  </>
                )}
                <Td pt={'12px'} textAlign={'center'}>
...
function EditLinkModal({
  ...
  return (
    <MyModal
      ...
      title={<Box>{isEdit ? publishT('edit_link') : publishT('create_link')}</Box>}
      maxW={['90vw', '350px']}
      w={'100%'}
      h={['90vh', 'auto']}
    >
      ...
      <ModalFooter>
        <Button variant={'whiteCommon'} w={'80px'} size={'sm'} mr={3} onClick={onClose}>
