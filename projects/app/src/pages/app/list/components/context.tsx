...
import { AppTypeEnum, sortTypeEnum } from '@fastgpt/global/core/app/constants';
...
type AppListContextType = {
  ...
  setSortType: React.Dispatch<React.SetStateAction<sortTypeEnum | undefined>>;
};
...
export const AppListContext = createContext<AppListContextType>({
  ...
  sortType: undefined,
  setSortType: function (value: React.SetStateAction<sortTypeEnum | undefined>): void {
    throw new Error('Function not implemented.');
  }
});
...
const AppListContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [sortType, setSortType] = useState<sortTypeEnum>(sortTypeEnum.updateTime);
  const {
    data = [],
    runAsync: loadMyApps,
    ...
    () => {
      const formatType = (() => {
        ...
        if (type === AppTypeEnum.plugin) return [AppTypeEnum.folder, AppTypeEnum.plugin];

        return [AppTypeEnum.folder, type];
      })();
      return getMyApps({ parentId, type: formatType, searchKey, sortType });
    },
    {
      manual: false,
      refreshDeps: [searchKey, parentId, type, sortType],
      throttleWait: 500
