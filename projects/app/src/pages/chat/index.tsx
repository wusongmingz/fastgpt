...
const Chat = ({
  ...
  const { appAvatar, appName } = useMemo(() => {
    return {
      appAvatar:
        llmModelList.find((item) => {
          return item.name === chatData.app.chatModels?.[0];
        })?.avatar || '',
      appName: chatData.app.chatModels?.[0]
    };
  }, [chatData, llmModelList]);
  ...
  const { loading: isLoading } = useRequest2(
    ...
    },
    {
      ...
      onError(e: any) {
        ...
        // reset all chat tore
        if (e?.code === 501) {
          router.replace('/applicationCenter');
        } else {
          router.replace({
            query: {
              ...router.query,
  ...
  const RenderHistorySlider = useMemo(() => {
    const Children = (
      <ChatHistorySlider
        ...
        appColor={chatData.app.selectColor}
  ...
  return (
    <Flex w={'100vw'} h={'calc(100vh - 80px)'}>
      <NextHead title={chatData.app.name} icon={chatData.app.avatar}></NextHead>
      {/* pc show myself apps */}
      {isPc && (
        <Box borderRight={theme.borders.base} w={'220px'} m={'10px'} mr={0} flexShrink={0}>
          <SliderApps apps={myApps} activeAppId={appId} />
        </Box>
      )}

      <PageContainer isLoading={loading} flex={'1 0 0'} w={0} p={[0, '10px']} position={'relative'}>
        <Flex h={'100%'} flexDirection={['column', 'row']} gap={'10px'}>
...
const Render = (props: Props) => {
  ...
  const { data: myApps = [], runAsync: loadMyApps } = useRequest2(
    () => {
      return getMyApps({ getRecentlyChat: true });
    },
    {
      manual: false
    }
  ...
  const providerParams = useMemo(
    () => ({ appId, source: ChatSourceEnum.online, searchName: searchKey }),
    [appId, searchKey]
  );
  return (
    <ChatContextProvider params={providerParams}>
      <Chat {...props} myApps={myApps} setSearchKey={setSearchKey} />
    </ChatContextProvider>
  );
};
