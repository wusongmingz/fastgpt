...
import { getCollectionSourceAndOpen } from '@/web/core/dataset/hooks/readCollectionSources';
...
const CollectionCard = () => {
  ...
  // const [iconName, setIconName] = useState<'asc' | 'desc'>('asc');
  ...
  const collectionId = useRef(collections[0]?._id);
  ...
  useEffect(() => {
    if (iconName === 'desc') {
      const newCollections = collectionsState.slice();
      ...
      const newCollections = collectionsState.slice();
      newCollections.sort((a, b) => b.name.localeCompare(a.name));
      setCollectionsState(newCollections);
      // if (iconTimeName === 'desc') {
      //   newCollections.sort((a, b) => {
      //     const time1 = new Date(a.updateTime);
  ...
  }, [iconName, iconTimeName]);

  const readSource = async (id: string) =>
    await getCollectionSourceAndOpen({
      collectionId: id
    })();
              ...
              <Tr>
                <Th py={4}>
                  ...
                  <MyIcon
                    ...
                    w={'16px'}
                    onClick={() => {
                      setIconName(iconName === 'desc' ? 'asc' : 'desc');
                    }}
                  />
                </Th>
                ...
                <Th py={4}>
                  ...
                  <MyIcon
                    ...
                    ml={1}
                    onClick={() => {
                      seticonTimeName(iconTimeName === 'desc' ? 'asc' : 'desc');
                    }}
                  />
                </Th>
                ...
                <Tr
                  ...
                  onClick={() => {
                    if (collection.type === DatasetCollectionTypeEnum.folder) {
                      ...
                      // console.log(collection._id, 1111);
                    }
                  }}
                >
                  <Td minW={'100px'} maxW={['200px', '250px']} draggable py={2}>
                  ...
                  <Td py={2} onClick={(e) => e.stopPropagation()}>
                    <Flex>
                      <Flex
                        alignItems={'center'}
                        onClick={() => {
                          readSource(collection._id);
                          // getCollectionSourceAndOpen({ collectionId: collection._id });
                          // console.log('查看');
                        }}
                      >
                        <MyIcon name={'look'} w={'28px'} h={'28px'} mr={2} />
                      </Flex>
                      <Flex
                        alignItems={'center'}
                        onClick={() => {
                      ...
                      >
                        <MyIcon name={'edit'} w={'30px'} h={'30px'} mr={2} color={'#404040'} />
                      </Flex>
                      <Flex alignItems={'center'}>
                        <MyIcon
                          mr={1}
