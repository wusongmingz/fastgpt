...
const FileSelector = ({
  ...
  const maxCount = type === 'graph' ? 1 : feConfigs?.uploadFileMaxAmount || 1000;
  const maxSize = (feConfigs?.uploadFileMaxSize || 1024) * 1024 * 1024;
  fileType = type === 'graph' ? '.txt' : fileType;
  const { File, onOpen } = useSelectFile({
    fileType,
    multiple: type === 'graph' ? false : true,
    maxCount
  });
  ...
  return (
    <MyBox
      ...
      {...props}
    >
      <MyIcon name={'common/uploadFileFill'} w={'62px'} />
      {isMaxSelected ? (
        <>
