...
import { getDatasets, resumeInheritPer } from '@/web/core/dataset/api';
...
import FolderModal from '../../knowledgeHome/component/Selective';
...
function List() {
  ...
  const itemBgImg = '/imgs/images/picture/datasetbackground.png';
  ...
  const tips = useRef('');
  const deleteTipsMap = useMemoizedFn(async (dataset: DatasetListItemType) => {
    if (dataset.type === DatasetTypeEnum.folder) {
      const apps = await getDatasets({
        parentId: dataset._id
      });
      if (apps.length > 0) {
        tips.current = '此目录不为空，请确认是否删除目录及其数据，删除后数据将无法恢复！';
      } else {
        tips.current = '请确认是否删除该目录？删除后数据将无法恢复!';
      }
    } else {
      tips.current = '请确认是否删除该知识库？删除后数据将无法恢复！';
    }
  });
  ...
  //   type: 'delete'
  // });
  const { openConfirm, ConfirmModal, onClose } = useDeleteModal();

  const onClickDeleteDataset = async (dataset: DatasetListItemType) => {
    await deleteTipsMap(dataset);
    openConfirm(
      () =>
  ...
  return (
    <>
      {(formatDatasets.length > 0 || !!searchKey) && (
        <Grid
          py={4}
          gridTemplateColumns={
            folderDetail
        ...
        >
          ...
          >
            ...
            />
            <AIcon name="icon-xinzeng" fontSize={'60px'} m={'auto'} color={'rgb(215,0,15)'}></AIcon>
          </Flex>
              ...
              >
                ...
                >
                  <HStack position={'relative'}>
                    {dataset.type === DatasetTypeEnum.folder && (
                      <Avatar src="/imgs/images/picture/文件.png" w={'30px'} my={1} mx={2} />
                    )}
                    <Box
                      // flex={1}
                      // maxW={'200px'}
                ...
                {dataset.type !== DatasetTypeEnum.folder && (
                  <Box
                    borderWidth={'20px'}
                    // borderColor={'red transparent transparent red'}
                    borderColor={`${dataset.selectColor} transparent transparent ${dataset.selectColor}`}
                    position={'absolute'}
                    top={0}
                    left={0}
                  />
      ...
      {myDatasets.length === 0 && (
        <EmptyTip pt={'35vh'} text={'暂无数据'} flexGrow="1"></EmptyTip>
        // <KnowledgeHome />
      )}
      ...
      {!!editPerDataset && (
        <ConfigPerModal
          ...
          managePer={{
            ...
            onDelOneCollaborator: (props: RequireOnlyOne<{ tmbId: string; groupId: string }>) =>
              deleteDatasetCollaborators({
                datasetId: editPerDataset._id,
                ...props
              }),
