...
export type DatasetContextType = {
  ...
  selectDatasetType: string;
  setSelectDatasetType: React.Dispatch<React.SetStateAction<any>>;
};
...
export const DatasetsContext = createContext<DatasetContextType>({
  ...
  selectDatasetType: '',
  setSelectDatasetType: function (value: React.SetStateAction<any>): void {
    throw new Error('Function not implemented.');
  }
});
...
function DatasetContextProvider({ children }: { children: React.ReactNode }) {
  ...
  const [selectDatasetType, setSelectDatasetType] = useState('time');
  const { parentId = null, selectType = 'ALL' } = router.query as {
    parentId?: string | null;
    selectType?: string;
  };
  // const [myDatasets, setMyDatasets] = useState<DatasetListItemType[]>([]);
  const order = ['folder', 'dataset', 'graph'];
  let {
    data: myDatasets = [],
    ...
    async () => {
      ...
      if (selectDatasetType) {
        if (selectDatasetType === 'type') {
          res.sort((a, b) => {
            return order.indexOf(a.type) - order.indexOf(b.type);
          });
        } else if (selectDatasetType === 'time') {
          console.log('time');
        ...
        } else if (selectDatasetType === 'fileName') {
          res.sort((a, b) => {
            return a.name.localeCompare(b.name);
          });
        }
      }
      if (selectType === 'ALL') return res;
      return res.filter((item) => {
        return item.type === selectType || item.type === 'folder';
      });
    },
    {
      manual: false,
      refreshDeps: [parentId, selectType, searchKey, selectDatasetType]
    }
  );
  ...
  // let [myDatasets, setMyDatasets] = useState<DatasetListItemType[]>([]);
