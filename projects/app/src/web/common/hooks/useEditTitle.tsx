...
export const useEditTitle = ({
  ...
  const EditModal = useCallback(
    ({
      ...
      return (
        <MyModal isOpen={isOpen} onClose={onClose} titleBgc="#e2e3ea" title={title} maxW={'500px'}>
          <ModalBody>
            {!!tip && (
          ...
          <ModalFooter>
            {!!closeBtnText && (
              <Button mr={3} variant={'whiteCommon'} onClick={onClose}>
                {closeBtnText}
              </Button>
            )}
            <Button onClick={runAsync} variant={'blackCommon'} isLoading={loading}>
