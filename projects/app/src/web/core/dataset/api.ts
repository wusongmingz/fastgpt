...
import { GraphMetaData } from './type';
...
export const getGraphStatus = (data: { fileId: string }) =>
  POST<{ status: string }>('/graphrag/ifSuccess', data);

export const getGraphMetaData = (data: { fileId: string }) =>
  POST<GraphMetaData>('/graphrag/graphSourceData', data);
